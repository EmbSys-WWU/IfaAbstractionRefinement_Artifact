...
It builds the analysis jar with Maven if it is missing or older than the sources, installs the transformer configuration next to it, and then runs the examples. It requires a JDK (25 or newer) on the `PATH`.
...
Natively, with the default 42 GB heap, "Transmitters" and "Leaky Transmitters" can be explored even without exploration refinement, but checking a policy on the resulting state space still runs out of memory.
`eval_native` also accepts a list of examples.

The output used for the paper is included as `evaluation_log.txt` in the main directory. It was produced with `eval_native` on a machine with 64 GB RAM (JDK 25.0.3, `-Xmx42g`).
...
1 GB is enough for every configuration, except "Transmitters" and "Leaky Transmitters" without exploration refinement.
